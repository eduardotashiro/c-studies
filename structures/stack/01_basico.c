...
int topo = -1;  // Variável que indica a "posição do topo" da pilha
...
void push(int valor)
{
    if (topo >= MAX - 1) // Verifica se a pilha já está cheia
    {
        printf("PILHA CHEIA!\n");
        return;
    }
...
int pop()
{
    if (topo == -1) // se o topo for 0 significa q a pilha ta vazia
    {
        printf("PILHA VAZIA!\n");
